...
def main_func(x,y,learning_rate = 0.005, num_epochs = 5,minibatch_size = 1000):
    ...
    weights = { 'wc1': tf.Variable(tf.random_normal([3, 3, 3, 32])),
                ...
                'wd1': tf.Variable(tf.random_normal([15*15*128, 256])),
...
#                writer.add_summary(summ,epoch)                
                minibatch_acc = sess.run(accuracy, feed_dict={X: batch_x,Y:batch_y,keep_prob: 1.0})
                ...
                print('cost'+str(minibatch_cost)+'train_acc'+str(minibatch_acc)+'test_acc'+str(test_acc))
...
costs,acc,test= main_func(dogs,breed,learning_rate = 0.005, num_epochs = 50,minibatch_size = 1000)
